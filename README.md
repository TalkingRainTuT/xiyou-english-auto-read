...
支持 **单词朗读、句子积累朗读、课文朗读、看义选词、角色扮演(翻译/问答)、故事复述、模仿朗读、趣味配音** 等多类；
...
| 课文朗读(整段) | `read` | 回放整段 → 评分 → `handleNext()` |
| 看义选词 | `chooseTranslateV2` | 直接点 `answer=true` 的正确选项 |
| 角色扮演(翻译/问答) | `paperDetail` | 用 System.Speech **TTS 合成答案**回放 → 关键词评分 |
| 模仿朗读/故事复述 | `paperDetail` | 回放匹配的**整段模型音频** → 发音评分 |
